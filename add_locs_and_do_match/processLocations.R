...
readKnownPlaceLists = function(placeListDir) {
	...
	# canadian provinces & u.s. territories, which parse just like states
    nonStatesFile = paste0(placeListDir, "/nonStatesTable.txt")
    nonStateAbbrevs = read.table(nonStatesFile, sep="\t", as.is=T)
    nonStateAbbrevs[,1] = tolower(nonStateAbbrevs[,1])
    colnames(nonStateAbbrevs) = c("stateLong", "stateAbbrev")
    ...
    countries1 = scan(countryFile1, what=character(), sep="\t", quote="", quiet=T)
    countries2 = scan(countryFile2, what=character(), sep="\t", quote="", quiet=T)
    countries3 = scan(countryFile3, what=character(), sep="\t", quote="", quiet=T)
    ...
    return(list(stateAbbrevs=stateAbbrevs, cityStateAbbrevs=cityStateAbbrevs, countryList=countryList, nonStateAbbrevs=nonStateAbbrevs))
}


# Returns inferredState, which can be any of: 51 state abbrevs; "USA" for other usa; "foreign"; "unparsed";
# NA for when there was no real text there.
# (Old: If we set breakOutKnownForeign = 0, then both "foreign" and "unparsed" become just "".)
# Behavior is unpredictable when string contains more than one location.
isLocationInUSA = function(locString, placeLists, breakOutKnownForeign = 1) {
    ...
    # The R base package regexes are erratic wrt non-ascii chars. The better move would be to rewrite the 
    # matching using stringi. (But that syntax is different, so I haven't changed most calls. Also, stringi is slow.)
    ...
    # trim any leading or trailing whitespace and trailing punctuation
	...
	# -metaInUSA: we don't (yet) know what state in US
	# -metaKnownUSLoc: we do know what state in US
	# -metaCityState: in fact, it matches that pattern
    metaInUSA = metaKnownUSLoc = metaCityState = metaForeign = F
    
    # if USA, set inferredStateAbbrev only at the end
    ...
    # again trim leading and trailing whitespace and trailing punctuation
    locString = sub("^\\s+", "", locString, perl=T)
    locString = stri_replace(locString, "", regex="[\\p{P}\\p{Z}\\p{M}]+$")
    # caution: sometimes the trailing punctuation was part of, e.g., "d.c." or "n.y.". Later we check for such.

    
    stub = isExactlyCityOrState(locString, placeLists$cityStateAbbrevs, placeLists$stateAbbrevs)
    # special case we handle out here: "LA" only should only count as "USA"
    if (locString == "LA") {
        stub$hasLoc = F
    ...
    stub = looksLikeCityState(locString, placeLists$stateAbbrevs) 
    ...
    foreignLoc = hasForeignLoc(locString, placeLists$countryList)
    if (foreignLoc) {
        metaForeign = T
    }
	# check for provinces & territories 
	if (!is.null(placeLists$nonStateAbbrevs)) {
		stub = isExactlyCityOrState(locString, placeLists$cityStateAbbrevs[0,], placeLists$nonStateAbbrevs)
		if (stub$hasLoc) {
			metaForeign = T
		}
		stub = looksLikeCityState(locString, placeLists$nonStateAbbrevs) 
		if (stub$hasLoc) {
			metaForeign = T
		}
	}
...
#     if (hasUSA || hasUSA2 || hasUS || isUS) {
#         # we might get more specific later, but record that we're at least in "USA"
#         inferredStateAbbrev = "USA"
#         metaInUSA = T
#     }
...
#     if (hasWCoast || hasECoast || hasMidWest || hasNEngland) {
#         inferredStateAbbrev = "USA"
#         metaInUSA = T
#     }
...
looksLikeCityState = function(locString, stateAbbrevs) {
    # Does it end in a state or state abbreviation (plus optional 5-digit zip code)? (case insensitive)
    #tokens = strsplit(locString, "\\W+", perl=T)[[1]]    # divide the string at non-word characters
    # delete apostrophes 
    locString = gsub("\\'", "", locString, perl=T)
    # delete periods if preceded and followed by a single letter
    #locString = gsub("\\b(\\w)\\.\\w\\b", "\\1", locString, perl=T)
    # oops! Bug in version 3, fixed below:
    locString = gsub("\\b(\\w)\\.(\\w)\\b", "\\1\\2", locString, perl=T)
    tokens = stri_split_regex(locString, "[^\\p{L}\\p{N}]+", perl=T, omit_empty=T)[[1]] # the stringi way
    isCityState = F
    ...
    if (length(tokens)) {
        ...
        if (finalWord %in% tolower(stateAbbrevs$stateAbbrev)) {
            ...
            isCityState = T
            # expect 2 NA's here
            #indices = match(c(finalWord, final2Words, final3Words), stateAbbrevs$stateLong)
            # greedy: precedence to longest
            # (dude! This is a real bug fix, 1/2016: when it took the shortest, it thought "West Virginia" meant "Virginia".)
...
handleSpecialCases = function(inferredStateAbbrev, locString) {
    ...
    if (inferredStateAbbrev %in% brazilStates) {
        ...
        if (!isBrazil) {
            if (inferredStateAbbrev == "SC") {
                ...
                # Rewriting with stringi to make the code more portable
                #isBrazil = grepl("\\b(Florian.polis|Joinville|Blumenau|S.o Jos.|Crici.ma|Chapec.|Itaja.|Jaragu.\\ do\\ Sul|Lages|Palho.a|Balne.rio\\ Cambori.|Brusque|Tubar.o)\\b", locString, ignore.case=T, perl=T, fixed=F)
    ...
    #spamLocations = c("Arizona Tea Distributing Co.", "19.  CO.", "From Brasil to CT")
    #if (locString %in% spamLocations) {
    #    inferredStateAbbrev = "spam"
    #}
...
hasForeignLoc = function(locString, countryList) {
    ...
    # example calls (simplest)
    # stri_cmp_equiv("hladny", "HLADNY", strength=2) ignores case diffs; use strength=1 to ignore diacritical marks
    # stri_endswith_coll("I'm from haiti", "Haïti", strength=1)
    
    # version 1: is it exactly a country or capital from my master list (modulo lower-case)?
    #t1 = mapply(stri_cmp_equiv, e1=locString, MoreArgs=list(e2=countryList, strength=2))
    # version 1b: what about if we remove everyone's diacritical marks?
    #t1b = mapply(stri_cmp_equiv, e1=locString, MoreArgs=list(e2=countryList, strength=1))
    
    # Result of sanity check: t2 (requiring the string to end in the country) works better than t1 or t1b.
    #     if (sum(t1) != sum(t1b) || sum(t1) != sum(t2)) {
    #         print(paste0("looksForeign(", locString, "): which(t1)=", paste(which(t1), collapse=" "), 
    #                      ", which(t1b)=", paste(which(t1b), collapse=" "), 
    #                      ", which(t2)=", paste(names(which(t2)), collapse=" ")))
    #     }
    
    
    # version 2: does it end in something on the list?
    # (This is the most correct way to do it, but it's too slow. :( )
#    t2 = mapply(stri_endswith_coll, pattern=countryList, MoreArgs=list(str=locString, strength=1))
    # is it faster if we drop the collation coolness? somewhat.
#    t2 = mapply(stri_endswith_coll, pattern=countryList, MoreArgs=list(str=tolower(locString)))
    
    # put it in ascii to match what writeInferredLocations did (though I'm still not certain I like it)
    locSimpler = stri_trans_general(tolower(locString), "latin-ascii")
#     t2 = mapply(stri_endswith_fixed, pattern=countryList, MoreArgs=list(str=locSimpler))
#     t3 = mapply(stri_startswith_fixed, pattern=countryList, MoreArgs=list(str=locSimpler))
#     return(sum(t2 | t3) > 0)   # say "foreign" using the most lenient criteria

    # make sure the country words are at word boundaries
    # Did we want to also allow words in the middle of the text? Maybe... but it wasn't the original intent.
    #countriesAsPatterns = paste0("\\b", countryList, "$|^", countryList, "\\b")

    countriesAsPatterns = paste0("\\b", countryList, "$|^", countryList, "\\b")
    # bug (but effects didn't seem that poor): forgot to use the pattern
    #t2 = mapply(stri_detect_regex, pattern=countryList, MoreArgs=list(str=locSimpler))
...
# Comments on unicode & stringi (moved out of the way here)
    ...
    # Do we want to use these to standardize text for locations? <-- no, I guess. (In the end, doing this to the country list though.)
    ...
    # Do we want to use these to test whether the text looks foreign? <-- yes: see hasForeignLoc.
...
# special cases to consider fixing: (this block done?)
...
# to fix:
# [x] Minn. for Minnesota; Mich; Mass; Miss; etc. --> Added list from http://www.stateabbreviations.us/ (though wish I'd made it a separate column)
# [x] Oklahoma City fell out of the city list 
# [x] cities: Shreveport, New Haven, Vegas
# places like PR: put them back, to identify as foreign
# what if state is just a substring? what if city is just a substring?
